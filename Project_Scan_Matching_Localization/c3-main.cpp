...
/**
 * @brief Perform Iterative Closest Point (ICP) alignment between a target and source point cloud.
 *
 * This function computes the transformation matrix that aligns the source point cloud to the target point cloud using
 * the PCL (Point Cloud Library) ICP implementation. It considers a starting pose for initialization and applies ICP for
 * a specified number of iterations.
 *
 * @param target The target point cloud (reference point cloud).
 * @param source The source point cloud (to be aligned to the target).
 * @param startingPose The initial pose of the source point cloud, including rotation (yaw, pitch, roll) and translation (x, y, z).
 * @param iterations The maximum number of iterations for the ICP algorithm.
 * @return Eigen::Matrix4d The resulting transformation matrix aligning the source to the target point cloud.
 */
...
Eigen::Matrix4d ICP(PointCloudT::Ptr target, PointCloudT::Ptr source, Pose startingPose, int iterations){
    ...
    if(icp.hasConverged()) {
        std::cout << "icp has converged! sore is " << icp.getFitnessScore() << std::endl;
        transformation_matrix = icp.getFinalTransformation().cast<double>();
        transformation_matrix = transformation_matrix * initTransform;
        return transformation_matrix;
    } else {
        std::cout << "icp does not converged!";
    }
