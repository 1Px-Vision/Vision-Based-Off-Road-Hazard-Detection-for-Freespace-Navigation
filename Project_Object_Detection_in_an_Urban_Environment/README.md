...
Based on the evaluation of three models for object detection in an urban environment, the SSD MobileNet V2 FPNLite demonstrated the best performance with an mAP@(0.5:0.05:0.95) IoU of 0.09543, surpassing both EfficientNet D1 and SSD ResNet50 V1 FPN. For detecting small objects such as cyclists and pedestrians, SSD MobileNet V2 FPNLite achieved the highest mAP, indicating superior capability in identifying smaller objects compared to the other models. On the other hand, EfficientNet D1 achieved the highest mAP for large objects, suggesting its strength in detecting larger objects, such as nearby cars.

All three models struggled with detecting cyclists, likely due to the dataset's class imbalance, where cars dominate and cyclists represent the least abundant class. Overall, the evaluation highlights that each model has distinct strengths and weaknesses in object detection. Selecting the most suitable model for a specific application requires careful consideration of the type and size of the objects to be detected. Furthermore, the results suggest that the ResNet50 model may require significant optimization and tuning to be a viable option for object detection in urban environments.
