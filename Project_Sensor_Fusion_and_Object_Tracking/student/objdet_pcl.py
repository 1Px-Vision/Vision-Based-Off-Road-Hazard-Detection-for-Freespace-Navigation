...
def show_range_image(frame, lidar_name):
    ...
    ri_intensity = np.amax(ri_intensity) * (0.99 - 0.01) * ri_intensity * 255 / (np.amax(ri_intensity) - np.amin(ri_intensity))

    # step 6 : stack the range and intensity image vertically using np.vstack and convert the result to an unsigned 8-bit integer
    ri_range_intensity = np.vstack((ri_range, ri_intensity))
    img_range_intensity = ri_range_intensity.astype(np.uint8) # remove after implementing all steps
    #######
    ####### ID_S1_EX1 END #######     
...
def bev_from_pcl(lidar_pcl, configs):
    ...
    intensity_map = np.zeros((configs.bev_height,configs.bev_width)) # bev_height and width = 608
    ...
    ## step 1 : create a numpy array filled with zeros which has the same dimensions as the BEV map
    height_map = np.zeros((configs.bev_height,configs.bev_width)) # bev_height and width = 608
